...
/datum/controller/subsystem/modpacks/Initialize()
	...
	for(var/datum/modpack/package as anything in all_modpacks)
		...
		if(QDELETED(package))
			auxtools_stack_trace("Modpack of type [package.type] is null or queued for deletion.")
		if(fail_msg)
			auxtools_stack_trace("Modpack [package.type] failed to pre-initialize: [fail_msg].")
		if(loaded_modpacks[package.name])
			auxtools_stack_trace("Attempted to register duplicate modpack name: [package.name].")
	...
	for(var/datum/modpack/package as anything in loaded_modpacks)
		...
		if(fail_msg)
			auxtools_stack_trace("Modpack [(istype(package) && package.name) || "Unknown"] failed to initialize: [fail_msg]")
	for(var/datum/modpack/package as anything in loaded_modpacks)
		var/fail_msg = package.post_initialize()
		if(fail_msg)
			auxtools_stack_trace("Modpack [(istype(package) && package.name) || "Unknown"] failed to post-initialize: [fail_msg]")
...
/datum/controller/subsystem/modpacks/proc/ui_static_data(mob/user)
	var/list/data = list()
...
/datum/controller/subsystem/modpacks/proc/ui_state(mob/user)
